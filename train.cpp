...
void NeuralNetwork::updateModelFile()
{
    ofstream modelOut(modelFilePath);
    for (int i = 0; i < neural_num; ++i)
	{
		modelOut << biases[0][i] << " ";
	}
	...
	for (int i = 0; i < input_num; ++i)
	{
		for (int j = 0; j < neural_num; ++j)
		{
			modelOut << weights[0][i][j] << " "; // / sqrt(double(input_num));
		}
        modelOut << endl;
	}
    modelOut << endl;
	...
	for (int i = 0; i < neural_num; ++i)
	{
		for (int j = 0; j < output_num; ++j)
		{
			modelOut << weights[1][i][j] << " "; // / sqrt(double(input_num));
		}
        modelOut << endl;
	}
}
